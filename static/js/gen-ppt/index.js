  ...
  for (var i = 0; i < arr.length; i++) {
    if (arr[i].parseType === 'scripture') {
      for (var j = 0; j < arr[i].parsed.length; j++) {
        for (var m = 0; m < arr[i].parsed[j].length; m++) {
          /*
            story: 每页放两节
          */
          if (m % 2 === 0) {
            ...
            */
            if (arr[i].parsed[j][m + 1]) {
              var txtArr1 = splitNumAndScripture(arr[i].parsed[j][m])
              txtArr1[1].text += '\r' //需要有一个换行符
              var txtArr2 = splitNumAndScripture(arr[i].parsed[j][m + 1])
...
function splitNumAndScripture(scripture) {
  var res = scripture.split(/(^\d+) /).slice(1, 3)
